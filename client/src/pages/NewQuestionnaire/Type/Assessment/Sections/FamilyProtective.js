...
/**
 * Family/Protective section of questionnaire. The components in the ./Sections directory all
...
const FamilyProtective = ({ answers, wrapperSetAnswers }) => {
  const [answersState, setAnswersState] = useState(answers) //Current answers obj
  const questions =
    answers.sections.protectiveFactors.familyProtective.questions //Questions array
  ...
  const onAnsInput = (e, qNum, name, isSub, comment) => {
    if (answersState) {
      ...
      if (comment) {
        tempAns.sections.protectiveFactors.familyProtective.generalComment =
          e.target.value
        ...
        if (!isSub) {
          if (document.querySelector('input[name="' + name + '"]:checked')) {
            ...
            tempAns.sections.protectiveFactors.familyProtective.questions[
              qNum
            ].answerBool = isTrue
            setAnswersState((answersState) => ({
              ...answersState,
  ...
  return (
    <>
      <h4 className="px-0 mt-4 mb-3">
        {answers.sections.protectiveFactors.familyProtective.name}
      </h4>
...
export default FamilyProtective
