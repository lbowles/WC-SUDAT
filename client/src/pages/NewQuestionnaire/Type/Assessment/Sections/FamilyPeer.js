...
/**
 * Family/Peer section of questionnaire. The components in the ./Sections directory all
...
const FamilyPeer = ({ answers, wrapperSetAnswers }) => {
  const [answersState, setAnswersState] = useState(answers) //Current answers obj
  const questions = answers.sections.familyCommunity.familyPeer.questions //Questions array
  ...
  const onAnsInput = (e, qNum, name, isSub, comment) => {
    if (answersState) {
      ...
      if (comment) {
        tempAns.sections.familyCommunity.familyPeer.generalComment =
          e.target.value
        ...
        if (!isSub) {
          if (document.querySelector('input[name="' + name + '"]:checked')) {
            ...
            tempAns.sections.familyCommunity.familyPeer.questions[
              qNum
            ].answerBool = isTrue
            setAnswersState((answersState) => ({
              ...answersState,
  ...
  return (
    <>
      <h4 className="px-0 mt-4 mb-3">
        {answers.sections.familyCommunity.familyPeer.name}
      </h4>
